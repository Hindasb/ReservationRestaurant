...
class Reservation_table(models.Model):
    ...
    nombre_personne_adulte = models.PositiveIntegerField(default=1)
    nombre_personne_enfant = models.PositiveIntegerField(default=1)
    chaise_enfant = models.BooleanField(default=False)
    information_sur_plats=models.CharField(max_length=500)

    
    def __str__(self):
       return f"{self.client.nom} {self.client.prenom} a reserver la table numero {self.table.numero} dans la salle numero {self.table.salle.numero} et le type d evenement est {self.type_evenement}, date de reservation: {self.date_reservation}"
  
class Reservation_salle(models.Model):
    Evenement_TYPE_salle = (
        ('anniversaire', 'anniversaire'),
        ('reunion_daffaire', 'reunion_daffaire'),
        ('Conference', 'Conference'),
    ...
    def __str__(self):
            return f"{self.client.nom} {self.client.prenom} a reserver la salle numero {self.salle.numero} et le type d evenement est {self.type_evenement}, date de reservation: {self.date_reservation}"
